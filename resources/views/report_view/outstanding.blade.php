...
@section('title', 'Students with Outstanding')

@section('content_header')
    <div class="report-page-header">
        <div>
            <h1 class="report-page-title mb-1">Students with Outstanding</h1>
...
<div class="card report-card">
    <div class="report-card-header">
        <h3 class="report-card-title">Outstanding List</h3>
    </div>
    ...
    <div class="report-card-body table-responsive">
        <table class="table table-bordered table-hover report-table">
            ...
            <tbody>
                @forelse($students as $index => $s)
                    <tr>
                        ...
                        <td>
                            ...
                            <br>
                            <small class="text-muted">{{ $s->middle_name }} {{ $s->extension_name }}</small>
                        </td>
                        <td>{{ $s->course }}</td>
                        <td class="text-center">{{ $s->year }}</td>
                        <td>
                            @if($s->remarks)
                                <span class="report-remark">{{ $s->remarks }}</span>
                            @else
                ...
                @empty
                    <tr>
                        <td colspan="6" class="report-empty">No outstanding students found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
