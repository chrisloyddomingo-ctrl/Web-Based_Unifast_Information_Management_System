...
@section('content_header')
    <div class="report-page-header">
        <div>
            <h1 class="report-page-title mb-1">Reports for PWD</h1>
        </div>
...
<div class="card report-card">
    <div class="report-card-header">
        <h3 class="report-card-title">PWD Students</h3>
    </div>
    ...
    <div class="report-card-body table-responsive">
        <table class="table table-bordered table-hover report-table">
            ...
            <tbody>
                @forelse($pwdStudents as $index => $p)
                    <tr>
                        <td class="text-center">{{ $index + 1 }}</td>
                        <td class="text-center">
                            <span class="report-badge">{{ $p->student_id }}</span>
                        </td>
                        <td>
                            <strong>{{ $p->last_name }}, {{ $p->given_name }}</strong>
                            <br>
                            <small class="text-muted">{{ $p->middle_name }} {{ $p->ext_name }}</small>
                        </td>
                        <td>{{ $p->program_name }}</td>
                        <td class="text-center">{{ $p->year_level }}</td>
                        <td class="text-center">
                            <span class="report-disability-badge">{{ $p->disability }}</span>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="report-empty">No PWD students found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="report-footer-note">
        Total Records: <strong>{{ $pwdStudents->count() }}</strong>
    </div>
</div>
@stop